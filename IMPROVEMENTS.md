...
### 2.10 `discover_servers` is an unconsented LAN port scan with no handshake *(medium)* — ✅ RESOLVED (Phase 3, UDP discovery)
...
### 3.5 Host self-join can collide with a client's `user_id` *(low)* — ✅ RESOLVED (canonical identity, v0.1.2)
...
**Resolved:** the host-as-authority **canonical identity** model removed the collision class entirely — every connector is upserted by email into the host DB to a globally-unique id, `server_streams`/`room_clients` are keyed by it, and the `is_server && user_id == server_user_id` broadcast skip was deleted. (See the Phase 3 "Multi-client delivery fix" entry.)
...
| ~~Direct/private messages~~ ✅ | high | DMs (1:1+group) + private channels shipped (v0.2.0) | L |
...
| ~~Settings/preferences persistence~~ ✅ | medium | Settings panel + `preferences.ts` (notifications/send-on-Enter/theme) | S |
...
- [x] Session restore: the login form now pre-fills the last-used username/email/department/mode/server from a persisted profile (`src/session.ts`); the room password (the Noise PSK) is deliberately never stored, and a returning user lands on the password field. — **L**
...
- [x] **Unread tracking + per-room badges** (migration v12 `user_rooms.last_read_at`; `get_unread_counts`/`touch_last_read`; host pushes `UnreadCounts` to clients on connect/join/activity, computes its own locally; Sidebar accent badges + bold unread rooms). Plus **socket `LoadOlder` pagination** (`HistoryRequest`/`HistoryPage` + `request_history`) so clients scroll past the initial 50. Works around the single-active-room relay by recomputing+pushing unread on chat activity. _Envelope `version` field (ADR-0004) **done** — `Message.version` + `PROTOCOL_VERSION` shipped._ — **M**
- [x] **Multi-client delivery fix — canonical identity** (resolves 3.5 + the `user_id`-collision skip in 3.1). The host upserts each connector **by email** into its own DB → a globally-unique canonical id, keys `server_streams`/`room_clients` by it, and re-stamps every frame's `user_id` to the connection-bound `auth_user_id` (stripping `email`). Fixes the cross-machine bug where a third participant sharing the host's local id could send but never receive; clients learn their canonical id via the `Identity` frame to attribute their own messages. (v0.1.2) — **M**
- [x] **Host-authoritative rooms for clients** — clients keep only seeded channels locally, so the host now PUSHES each client its real room list (`RoomList`) on connect + on every membership change, and routes **client-created channels and DMs through itself** (`RoomCreate`/`DmRequest` → create on host DB under the canonical id → `RoomReady`/`DmReady` to open), with `ErrorNotice` feedback on failure (e.g. duplicate name). Clients no longer write room membership to their local DB. Fixes cross-machine DMs, private-channel invites, and client-created channels. (v0.2.1–v0.2.2) — **M**
- [x] **Invite UI + private-channel membership** — searchable member picker (`InviteModal`) backed by a host-pushed `UserDirectory` (`{id,name,is_online}`); private channels are scoped to **creator + invited members** (`add_room_member`, `room_join_allowed_internal`); `Chat`/`RoomJoin`/`HistoryRequest` are all membership-gated so a crafted frame can't inject into or read a private room/DM (2.2/2.9). (v0.2.0) — **M**
- [x] **DMs (1:1 + group)** — a DM is a private room (`is_dm`, migration v13) with a fixed member set; 1:1 uses a deterministic name (find-or-create), groups get a fresh id; the UI shows a display name derived from the other members. `NewDmModal` picker; right-aligned own-message bubbles distinguish your messages. Resolves the `is_private`-plumbed-but-ungated gap in the §7 table. (v0.2.0) — **L**
- [x] **Working discovery in UI (UDP announce/respond, replaces the port scan) (7/2.10)** — a versioned UDP probe/respond on udp/3626 (magic + `DISCOVERY_VERSION`): only well-formed Nutler announces are listed (no false positives), it's consent-gated (a "Find hosts on your network" button), carries no PSK, and the host responder advertises real name + live user count. The responder is hardened against reflection/amplification (LAN-source-only, name length cap, per-source reply cooldown) and self-listing (per-scan nonce). Trust stays the Noise handshake on connect. Adversarially reviewed (4 dimensions, find→verify). — **M**
- [x] Typing indicators (ephemeral `MessageType::Typing`, host-relayed, never persisted; throttled send + peer-side expiry; animated "X is typing…" row). — **M**
- [x] **Settings/preferences persistence (7)** — a Settings panel (gear in the sidebar footer, `SettingsModal`) backed by `src/preferences.ts` (mirrors `session.ts`: single `nutler.preferences` key, validated load, allowlisted save, valid defaults). Preferences: **desktop notifications** (all/mentions/off — the notification gate reads the level via a ref; default preserves prior behavior), **send-with-Enter** (Cmd/Ctrl+Enter always sends), and **theme** (reuses the single `useTheme` instance). Adversarially reviewed (4 dims). — **S**

**Phase 3 is complete** — all feature-expansion items shipped except server discovery's optional mDNS variant (UDP announce/respond covers the requirement). Remaining across the whole roadmap: Phase 4 **PII-at-rest** (2.11) and internal-only deferrals (god-hook split 5.5, per-message rate-limit, typed AppError codes 8.7, ghost-member eviction).

### Phase 4 — Polish, tests, CI, release
- [x] Rust `tokio::test` integration tests over an in-memory SQLite pool with the real migrations applied: pagination/cursor ordering, author-scoped edit/delete (+ edit-after-delete no-op), reaction toggle/aggregate/`me` flag, and idempotent save (`db_queries.rs` test module). **Frontend Vitest + RTL added** (`vitest.config.ts`, `src/test/setup.ts`): 26 tests over `utils`, `session` (incl. corrupt-profile + no-secret-leak), and a `LoginView` RTL test (submit-gating, profile pre-fill, stale-department reconcile). — **L**
- [x] CI `check` workflow on PRs/push (`.github/workflows/ci.yml`): frontend tsc + **eslint** + `prettier --check` + **vitest** + build; backend `cargo fmt --check` + `cargo clippy -D warnings` + `cargo test`, with `npm ci`, `libwebkit2gtk-4.1`, Node 20, rust-cache, and run-cancellation. Clippy clean at `-D warnings`. Also fixed the release workflow's `libwebkit2gtk-4.0 → 4.1`. **Release-action bumped to `action-gh-release@v3`** (with the rest of the Dependabot sweep, below). — **M**
- [x] **Dependency modernization (resolves 8.4 + the full Dependabot backlog).** All 12 Dependabot PRs handled/superseded and merged: npm — vite 7→**8** (+ vitest 2→4, plugin-react 4→6 to clear the vite-8 peer conflict), eslint 9→**10**, lucide-react 0→**1**, `@types/node` 24→**26**, in-range minor/patch group; Rust — sqlx 0.8→**0.9**, snow 0.9→**0.10** (Noise transport; `psk()` made fallible — handshake wire-identical, verified by the `secure.rs` round-trip + wrong-PSK tests), sha2 0.10→**0.11**; GitHub Actions — checkout 7, setup-node 6, upload-artifact 7, **action-gh-release 3**. Each built/clippy'd/tested; a cross-platform lockfile fix was needed for vite-8's native deps. Also: the release upload now ships only installers (extension-matched glob), keeping tauri's dmg helper files out of releases. — **M**
- [x] **ESLint** wired (flat config, classic react-hooks rules; the v7 React-Compiler rules left opt-in). _`tracing` logging done (below); pre-commit hook still optional._ — **M**
- [x] **`tracing` logging** (8.6): added `tracing` + `tracing-subscriber` with an `EnvFilter` (default INFO = prior behavior, `RUST_LOG`-overridable); converted all 82 `println!`/`eprintln!` in `sockets.rs` to `tracing::info!`/`error!`. — **M**
