...
- [x] CI `check` workflow on PRs/push (`.github/workflows/ci.yml`): frontend tsc + **eslint** + `prettier --check` + **vitest** + build; backend `cargo fmt --check` + `cargo clippy -D warnings` + `cargo test`, with `npm ci`, `libwebkit2gtk-4.1`, Node 20, rust-cache, and run-cancellation. Clippy clean at `-D warnings`. Also fixed the release workflow's `libwebkit2gtk-4.0 → 4.1`. _Release-action bump still pending._ — **M**
...
- [x] Governance docs: `CONTRIBUTING.md`, `CHANGELOG.md`, `SECURITY.md`, `.github/dependabot.yml` (npm + cargo + actions), **issue/PR templates** (`.github/ISSUE_TEMPLATE/*`, `pull_request_template.md`). _Codesign/notarize + `deploy.sh` review still pending._ — **M**
