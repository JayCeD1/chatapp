...
9. **Sha grants retrieval via Chat-ref laundering (Phase 6 release-gate decision).** `validate_chat_attachments` checks existence+size only; any authenticated member can reference any stored sha in a room they belong to and then fetch it via their own attachment id — and the `UploadStart` dedup fast-path is a content-existence oracle by sha. This makes §6's "knowing a hash grants nothing" currently false (practical severity low: a 256-bit hash must leak out-of-band). Before release, either scope validation/dedup to shas the sender uploaded or that already appear in a room they can access, or amend §6/§8 to explicitly accept it.
...
Backlog (pre-existing, surfaced during this review): the client listener logs the full plaintext of every non-attachment frame at info level (`sockets.rs` "Client received") — against the never-log-plaintext spirit; fix as its own change.
